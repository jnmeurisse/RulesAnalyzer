...
namespace fwo {
	...
	void ModelConfig::load(const toml::Table& config_table)
	{
	...
	void ModelConfig::throw_invalid_parameter(const std::string& key, const std::string& message)
	{
	...
	bool ModelConfig::has_key(const toml::Table& table, const std::string& key)
	{
	...
	void ModelConfig::load_int(const toml::Table& table, const std::string& key, int& value)
	{
	...
	void ModelConfig::load_bool(const toml::Table & table, const std::string & key, bool& value)
	{
		if (has_key(table, key)) {
	...
	void ModelConfig::load_string(const toml::Table & table, const std::string & key, std::string& value)
	{
		if (has_key(table, key)) {
	...
	void ModelConfig::load_char(const toml::Table & table, const std::string & key, char& value)
	{
		if (has_key(table, key)) {
			const auto result = table.getString(key);
