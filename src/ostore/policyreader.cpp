...
namespace fos {
	...
	PolicyReader::PolicyReader(std::istream& csv_stream, const csv_reader_config& reader_config, f_interrupt_cb interrupt_cb) :
	...
	bool fos::PolicyReader::next_row(RuleObject& rule)
	{
		...
		if (!str2i(values[value_index++], rule.id) || rule.id <= 0) {
		...
		const char list_delimiter = _reader_config.list_delimiter;
		if (split(values[value_index++], list_delimiter, rule.source_zones) == 0) {
			_logger->error("row %d : source zones is empty", _reader.row_number());
			valid = false;
		}

		if (split(values[value_index++], list_delimiter, rule.source_addresses) == 0) {
			_logger->error("row %d : source addresses is empty", _reader.row_number());
			valid = false;
		}

		if (!str2b(values[value_index++], rule.negate_source_addresses)) {
			_logger->error("row %d : negate source addresses is not a boolean", _reader.row_number());
			valid = false;
		}

		if (split(values[value_index++], list_delimiter, rule.destination_zones) == 0) {
			_logger->error("row %d : destination zones is empty", _reader.row_number());
			valid = false;
		}

		if (split(values[value_index++], list_delimiter, rule.destination_addresses) == 0)  {
			_logger->error("row %d : destination addresses is empty", _reader.row_number());
			valid = false;
		}

		if (!str2b(values[value_index++], rule.negate_destination_addresses)) {
			_logger->error("row %d : negate destination addresses is not a boolean", _reader.row_number());
			valid = false;
		}

		if (split(values[value_index++], list_delimiter, rule.services) == 0) {
			_logger->error("row %d : services is empty", _reader.row_number());
			valid = false;
		}

		if (split(values[value_index++], list_delimiter, rule.applications) == 0) {
			_logger->error("row %d : applications is empty", _reader.row_number());
			valid = false;
		}

		if (split(values[value_index++], list_delimiter, rule.users) == 0) {
