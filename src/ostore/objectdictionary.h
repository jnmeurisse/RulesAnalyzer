...
namespace fos {
	...
	public:
		ObjectDictionary(const std::string& name, const csv_reader_config& reader_config);
		...
		*/
		virtual const T* get(const std::string& name) const noexcept;
	...
	protected:
		...
		*/
		...
		const OstoreConfig& options() const noexcept { return _options; }
	...
	private:
		...
		const csv_reader_config& _reader_config;
	...
	template<typename T>
	inline ObjectDictionary<T>::ObjectDictionary(const std::string& name, const csv_reader_config& reader_config) :
		_name{ name },
	...
	template<typename T>
	const T* ObjectDictionary<T>::get(const std::string& name) const noexcept
	{
		auto it = _items.find(name);
	...
	{
		return ::split(str, _reader_config.list_delimiter);
	}
