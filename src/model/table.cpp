...
namespace fwm {
	...
	{
		assert(("invalid table size", headers.size() == wrap_positions.size()));
	...
	{
		assert(("invalid row index", row_idx < _rows.size()));
	...
	Table& Table::merge(const Table& other)
	{
		if (other._rows.size() != _rows.size())
			throw std::runtime_error("table can not be merged, number of rows is different");
	...
	size_t Table::col_width(size_t col_idx) const noexcept
	{
		assert(("invalid column index", col_idx < _headers.size()));
	...
	{
		assert(("invalid column index", col_idx < _cells.size()));
	...
	size_t Row::height() const noexcept
	{
		size_t max_height{ 0 };
	...
	size_t Row::width(size_t col_idx) const noexcept
	{
	...
	const std::string& Cell::line(size_t line_idx) const noexcept
	{
	...
	{
		return strings_join(_lines, delim, false);
	}
