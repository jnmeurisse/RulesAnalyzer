...
namespace fwm {
	...
	Table RuleAnomalies::output_anomalies(bool show_rule_name) const
	{
		...
		for (auto& anomaly : *this) {
			...
			anomaly->rule().predicate().src_zones().output(row->cell(col_num++), MnodeInfoType::NAME);
			anomaly->rule().predicate().dst_zones().output(row->cell(col_num++), MnodeInfoType::NAME);
