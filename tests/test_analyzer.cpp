...
static PredicatePtr create_predicate(fwm::Network& network,
	std::string src,
	std::string dst,
	std::string svc,
	...
	bool negate_dst = false)
{
	...
	PredicatePtr p = std::make_unique<const Predicate>(
		Sources{ sz, sa, negate_src },
		Destinations{ dz, da, negate_dst },
		services,
		applications,
		users);
...
TEST(Analyzer, any) {
	...
	Network network;
	network.register_src_address("corporate_net", "10.0.0.0/8");
	network.register_dst_address("corporate_net", "10.0.0.0/8");


	// Configure the test firewall
	network.add(std::make_unique<Firewall>("test", network));
	...
	firewall->add_rule(std::make_unique<Rule>(
		"rule1",
		1,
		RuleStatus::ENABLED,
		RuleAction::ALLOW,
...
TEST(Analyzer, shadowing) {

	// Define network objects
	Network network;
	network.register_src_address("R_10.1.1.0/25", "10.1.1.0/25");
	network.register_src_address("R_10.1.1.128/25", "10.1.1.128/25");
	network.register_src_address("R_172.16.1.0/24", "172.16.1.0/24");
	network.register_dst_address("R_192.168.1.0/24", "192.168.1.0/24");
